...
When committing a large batch of new files spanning multiple disciplines, split commits by discipline (geometry, propulsion, aerodynamics, weights each separately; `run_all_tests` its own commit too), and never commit `.asv`, `__MACOSX`, `temp_Casey`, or scratch working docs — but DO commit `docs/reference_extracts/` (the moved reference-data extracts that code cites).
...
Concretely, for F-16 aerodynamics at L1: `AerodynamicsBase` (abstract) ← `AeroModelL1` (abstract) ← `F16AeroL1` (concrete, in `examples/F16A/models/disciplines/aero/`), with equations living in the standalone `AeroL1` toolbox that `F16AeroL1` calls.

**F-16A example directory layout (reorganized 2026-08-12).** The `examples/F16A/` tree is grouped by role, not flat. Concrete `F16*L*` Tier-3 classes + their same-basename companion `.md` docs live under `examples/F16A/models/disciplines/<discipline>/` (`aero`, `geom`, `prop`, `weights`, `tail`, `sandc`, `subsystems`, `landing_gear`); `F16ConstraintSet.m` + the `design_study_0*.m` drivers under `examples/F16A/models/sizing/`. Path helpers (`f16a_spec_path.m`, `f16a_requirements_path.m`) are in `examples/F16A/helpers/`; input JSONs (`f16a_L*.json`, `f16a_requirements.json`) + the requirements companion doc in `examples/F16A/inputs/`; run-scripts (`run_sizing_report_L*.m`, `run_F16_constraint_diagram.m`) in `examples/F16A/studies/`; and the Brandt comparison reports (`*_brandt_comparison.m`, plus the renamed `mission_brandt_comparison.m`) in `examples/F16A/sanity_checks/`. Their generated `.json/.md` output is written to the **gitignored** `examples/F16A/output/` (only a tracked `.gitkeep` keeps the folder present). Removed in the same pass: `Mission_Profile.xlsx` (mission profiles now live in `f16a_requirements.json` `.missions`), `fidelity_comparison.m` (the per-discipline `sanity_checks` reports supersede the cross-discipline grid), `run_F16_constraint_diagram_overlay.m`, and two scratch `*_scrape.md` files.
...
There is still **no L3 propulsion tier** (no `PropL3`/`PropulsionModelL3`/`F16PropL3`), and none is planned: the L3 rung pairs `F16AeroL3` with `F16PropL2`, and anything reporting L3 propulsion numbers must label them "computed by `F16PropL2`". See `sizing/examples/F16A/models/disciplines/geom/F16GeomL3.md` for the full L3 geometry writeup and `sizing/src/disciplines/geometry/GeomL2.md` for the historical L2-merge note.
...
Do **not** compute a derived quantity once in the constructor and freeze it into a plain property — that silently goes stale under mutation (exactly the bug this pattern removes). Derived properties are read-only (no set-method); assigning to one errors, which is correct — they're outputs. The formulas are cheap closed-form algebra, so recompute-on-read costs nothing measurable and buys correctness-by-construction with zero cache bookkeeping. `examples/F16A/models/disciplines/geom/F16GeomL2.m` is the reference implementation — see its header block for the full rationale. This also dovetails with the JSON split: only true inputs live in the input JSON; derived quantities are never stored there, they're computed by the Dependent getters.
