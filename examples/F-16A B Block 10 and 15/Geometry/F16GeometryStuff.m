classdef F16GeometryStuff < GeometryEstModel
     ...
     methods
          ...
          function S_wet = get_S_wet(obj, design)
               S_wet = calc_S_wet_III(obj, design.WeightResults.W_TO);
          end

          % Size the tail
          function [S_HT, S_VT] = size_tail(obj, design)
               S_wet = get_S_wet(obj, design);
               [S_HT, S_VT] = Tail_Sizing_IV(obj, design.geom.wings.VerticalTail.c_VT, design.geom.wings.HorizontalTail.c_HT, design.geom.wings.Main.Spanft, S_wet, design.geom.fuselage.Fuselage.Lengthft, design.geom.wings.Main.MeanGeometricChord);
          end
     end
