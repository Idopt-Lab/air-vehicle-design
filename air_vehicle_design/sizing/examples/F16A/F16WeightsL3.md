...
| **All 62 §15.3.1 exponents are unverified against the printed book** — tallied 2 CONFLICT / 9 FROM-CODE / 24 VERIFY / 27 IMAGE-ONLY / 5 extract-clean in `WeightsL3.m`'s header. The two CONFLICTs keep their **code** values: Eq. 15.13 `N_en^1.023` (extract says 1.078), Eq. 15.3 `cos(Λ_vt)^−0.323` (extract says −1.0). Do not change a value to make the guard green | `TestWeightsL3.testTODO_Raymer1531ExponentsNotBookVerified` — deliberately red; todo §3a |
