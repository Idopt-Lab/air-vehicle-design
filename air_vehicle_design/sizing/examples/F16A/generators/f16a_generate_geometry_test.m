function [T, G] = f16a_generate_geometry_test()
...
%              configuration. The seed W_TO was a 45,000 lbf guess until
%              8/12/2026; it is Brandt's own converged 31,377 lbf now, so a
%              downstream divergence is a method difference rather than a
%              weight difference carried forward.
...
%
%   SECOND OUTPUT G (added 8/12/2026): the cascade's three geometry objects
%   (G.L1/L2/L3) and the scalars they were built from, so a downstream
%   discipline can use this geometry instead of a re-guess. First consumer:
%   f16a_generate_aerodynamics_test.
...
W_TO                  = 31377.0;  % lbf  [Brandt Main! mission W_TO_lb, his converged TOGW]
...
% as SizingLoopL2/design_study_02_L2/03_L3. size() is a 5-scalar call (was
...
% TODO (8/12/2026): Brandt optimum values should be updated to most recent
% values.
...
T = [T; ComparisonReport.row('S_ref, bootstrapped = W_TO/(W/S)_opt [ft^2]', 'L1', S_ref, 300.0, ...
    'Brandt Main!B18', '%.2f', 'No longer a hand-typed given -- Stage 0''s W_TO divided by the constraint-diagram optimum above.')];
...
% The cascaded geometry itself, for a downstream discipline to consume. The
% table rides along as G.T, so fidelity_comparison can stack every discipline
% from one run of the chain.
G = struct('L1', g1, 'L2', g2, 'L3', g3, 'W_TO', W_TO, 'S_ref', S_ref, ...
    'AR_wing', AR_wing_L1, 'lambda_wing', lambda_wing, 'LE_sweep_wing', sweep_LE_wing_graphical, ...
    'T', T);
