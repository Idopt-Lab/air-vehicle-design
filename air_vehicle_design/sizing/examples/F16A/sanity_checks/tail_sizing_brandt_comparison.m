...
%   L1 tail sizing takes S_ref/b/cbar and the two tail moment arms as raw
%   scalars (GeometryModelL1 has no planform to inject); L2 tail sizing takes
%   the injected g2 object directly and reads its geometry live. The arms
%   passed to L1 here are g2's own Dependent L_HT/L_VT -- see the preamble
%   note below on the 2026-08-11 arm-definition correction.
%
...
%    The last two arguments are g2's own Dependent tail moment arms (the
%    wing-mac-c/4 to tail-mac-c/4 distances, Raymer 6th ed. Sec. 6.5.2,
%    p.158). Before 2026-08-11 this passed g2.L_fus and TailL1 turned it into
%    0.475*L_fus = 22.09 ft -- Raymer's pre-layout stand-in for the same
%    quantity, +46% against this airframe's real 15.09 ft.
tail1  = F16TailL1();
r1     = tail1.size(g2.S_ref, g2.b_wing, g2.cbar_wing, g2.L_HT, g2.L_VT);
...
T = [T; grow('S_ht, L2 Nicolai/Carichner F-16 coefficient [ft^2]', 'L2', r2.S_ht, S_ht_brandt, 'Brandt Main!C18', '%.4f', ...
    ['Nicolai & Carichner Table 11.6, "General Dynamics F-16" row (C_HT=0.3, an F-16-SPECIFIC measured ' ...
     'coefficient, not a generic category row). Since 2026-08-11 L2 also uses Nicolai''s OWN arm definition ' ...
     '(Eqs. 11.1/11.2: initial c.g. estimate to the tail mac quarter-chord, with the initial c.g. at 30% of ' ...
     'the wing mac per Nicolai Ch. 8, p.212) rather than L1''s Raymer wing-c/4 reference. The two arms differ ' ...
     'by 0.05*cbar = 0.57 ft here, ~3.9%; each is used with the coefficient table it belongs to.'])];
...
T = [T; grow('S_lef, leading-edge flap [ft^2]', 'L1', cs.S_lef, 36.71, 'T.O. 1F-16A-1 Fig. 1-2', '%.4f', ...
    ['c/c=0.1157, eta=0.00-0.98. UPDATED 2026-08-11: c/c WAS 0.15, an uncited estimate carried over from ' ...
     'F16AeroL3. It is now MEASURED -- cf=1.31 ft, the LEF panel''s own physical chord read off Brandt''s ' ...
     'Main-sheet geometry chart ("LE Flap" series), over cbar_wing=11.32 ft. That is ground truth of the ' ...
     'same standing as a T.O. figure, not an estimate fitted to close a gap. eta_in=0 stays physically ' ...
     'generous (the real LEF starts OUTBOARD of the strake) and is deliberately NOT adjusted, which is why ' ...
     'this row still misses. Logged in VnV/BrandtF16A/todo.md.'])];
...
meta.preamble = { ...
    [sprintf(['**TAIL MOMENT ARM CORRECTED 2026-08-11.** Every number in this table before that date was ' ...
     'produced with an arm of 0.475*L_fus = %.2f ft. That is Raymer''s PRE-LAYOUT approximation (6th ed. ' ...
     'Sec. 6.5.2, p.159-160) of an arm he DEFINES on p.158 as the "tail-quarter-chord to wing-quarter-chord ' ...
     'distance"; Nicolai & Carichner Eqs. (11.1)/(11.2) define the same arm from the initial c.g. estimate ' ...
     '(placed at 30%% of the wing mac, Nicolai Ch. 8, p.212) instead. Built from this airframe''s own ' ...
     'stations [Brandt Main!B23/C23/H23], the arms are L_HT = %.4f ft and L_VT = %.4f ft (Raymer reference) ' ...
     'and %.4f / %.4f ft (Nicolai reference) at the S_ref = %.0f ft^2 baseline. The old fraction overstated ' ...
     'the HT arm by %+.1f%%, and S_HT is inversely proportional to it, so it understated both tail areas by ' ...
     'that same factor. Nothing here was tuned to close the gap to Brandt: only the arm''s DEFINITION was ' ...
     'corrected, and the volume coefficients are untouched.'], ...
     TailL1.compute_tail_arm(g2.L_fus), g2.L_HT, g2.L_VT, ...
     TailL2.compute_tail_arm_cg(g2.x_c4_ht, TailL2.compute_x_cg_initial(g2.x_mac_le_wing, g2.cbar_wing)), ...
     TailL2.compute_tail_arm_cg(g2.x_c4_vt, TailL2.compute_x_cg_initial(g2.x_mac_le_wing, g2.cbar_wing)), ...
     g2.S_ref, 100*(TailL1.compute_tail_arm(g2.L_fus) - g2.L_HT)/g2.L_HT)], ...
