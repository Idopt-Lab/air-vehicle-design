classdef F16MissionL2 < MissionModelL2
%F16MISSIONL2  F-16A CAP mission Level-2 student class.
%
%   Inherits from MissionModelL2 (abstract enforcer). compute_fuel is a
%   single delegation line to MissionL2.get_mission_fuel -- no equations are
%   duplicated here.
%
%   Level-2 model: single-point Breguet (Roskam Eq. 2.10/2.12 forms) with
%   REAL aero/prop discipline objects for Cruise/Dash/Combat/Loiter; Roskam
%   Table 2.1 fixed fractions (unchanged from L1) for Startup/Taxi/Takeoff/
%   Climb/Descent/Landing -- see MissionModelL2's header.
%
%   ============================================================================
...
%   (override by passing an explicit path) -- the SAME profile L1 reads
%   (mission_profile.json's schema is identical across fidelity levels;
...
%     [JSON]     examples/F16A/mission_profile.json
    ...
    properties
        ...
        RFF
        aircraft_category = "fighter"   % Roskam Table 2.1 category key, still needed for the fixed-fraction phases (see MissionModelL2's header)
    ...
    methods
        ...
        function obj = F16MissionL2(json_path)
        %F16MISSIONL2  Construct from a required mission-profile JSON path
        ...
        %COMPUTE_FUEL  Single delegation into MissionL2.get_mission_fuel.
        %   aero/prop are the REAL discipline objects -- called once per
        %   segment for Cruise/Dash/Combat/Loiter (see MissionModelL2's header).
            W_fuel = MissionL2.get_mission_fuel(obj.missiondata, W_TO, aero, prop);
