classdef F16AeroLevel3 < AerodynamicsModelLevel3
     %F16AEROLEVEL3 F-16 Level-3 aerodynamics concrete.
     %   Implements the AerodynamicsModelLevel3 contract with F-16 state.
     %   Equations come from the static toolboxes (AeroLevel1/2/3, AeroUtils)
     %   via ClassName.<method>(...); this class holds data + wiring.
     % Should compute:
     %    - drag (CD, CD0 [sub & sup])
     %    - lift
     ...
     properties (Constant) % These should be values that are tabulated based on geometry.
          ...
          E_WD = 2.2; % Equivalent wave drag parameter
     ...
     methods
          ...
          function K1 = compute_K1(aero_obj, M, AR, e_osw, LE_sweep_deg)
               if (0.0 < M) && (M < 1.0)
                    K1 = AeroLevel1.K1_sub(AR, e_osw);
               elseif (M >= 1.0)
                    K1 = AeroLevel1.K1_sup(AR, M, LE_sweep_deg);
               else
          ...
          function K2 = compute_K2(aero_obj, M, K1, CLminD)
               if (0.0 < M) && (M < 1.0)
                    K2 = AeroLevel1.K2_sub(K1, CLminD);
               elseif (M >= 1.0)
                    K2 = AeroLevel1.K2_sup();
               else
                    error("Error handler.")
               end
          end
          ...
          function output = eta_mach(obj, cl_alpha, beta_mach)
               output = AeroLevel3.eta_mach(cl_alpha, beta_mach);
          end
          ...
          function CD = get_CD(aero_obj, CD0, CDi, CL, CL_minD, airfoiltype, statevector, K1)
               if airfoiltype == "uncambered"
                    CD = AeroLevel2.compute_CD_uncambered(CD0, K1, CL);
               elseif airfoiltype == "cambered"
                    M = statevector(1);
                    if M >= 1.0
                         CD = AeroLevel2.compute_CD_cambered(CD0, K1, CL, CL_minD);
                    elseif M < 1.0
                         CD = AeroLevel2.compute_CD_cambered(CD0, K1, CL, CL_minD);
                    else
          ...
          function CDi_design = get_CDi(aero_obj, statevector, S_ref, e_osw, AR, L)
               ...
               if M >=1.0
                    ...
                    CDi_design = AeroLevel1.CDi_supersonic(CL, alpha_deg);
               elseif M<1.0
                    % Subsonic
                    CL = AeroLevel1.CL(L, q, S_ref);
                    CDi_design = AeroLevel1.CDi_subsonic(CL, e_osw, AR);
               else
          ...
          function cl_alpha = get_cl_alpha(aero_obj, M)
               if (0.0 < M) && (M < 1.0)
                    cl_alpha = AeroLevel3.CL_alpha_2D_sub(M);
               elseif (1.0 <= M)
                    cl_alpha = AeroLevel3.CL_alpha_2D_sup(M);
               else
          ...
          function output = get_CL_alpha(aero_obj, M, cl_alpha, AR, S_exposed, S_ref, F, Lambda_max_t_deg)
               if (0.0 < M) && (M < 1.0)
                    ...
                    output = AeroLevel3.CL_alpha_wing_sub(AR, S_exposed, S_ref, F, Lambda_max_t_deg, beta_mach, eta_mach);
               elseif (1.0 <= M)
                    beta_mach = sqrt(M^2 - 1);
                    output = AeroLevel3.CL_alpha_wing_sup(beta_mach);
               else
          ...
          function Delta_CD0 = get_Delta_CD0(aero_obj, flaptype, cf_c, S_flapped, S_ref, delta_flap_deg)
               ...
               % NOTE: Delta_CD0_flap is not defined in this class or any
               % toolbox for L3. Pre-existing gap - needs an L3 Delta_CD0_flap.
          ...
          function Delta_CDi = get_Delta_CDi(aero_obj, areFlapsFullOrHalfSpan, Delta_CL_flap, Lambda_cbar_q_deg)
               if (ismember(areFlapsFullOrHalfSpan, ["full"]))
                    k_f = 0.14;
               elseif (ismember(areFlapsFullOrHalfSpan, ["half"]))
                    k_f = 0.28;
               else
                    error("Error handler.")
               end

               % NOTE: Delta_CDi_flap is not defined for L3. Pre-existing gap.
          ...
          function Delta_CL_max = get_Delta_CL_max_values(aero_obj, Delta_cl_max, S_flapped, S_ref, Lambda_HL_deg)
               ...
               % NOTE: Delta_CL_max is not defined for L3. Pre-existing gap.
          ...
          function DragResults = get_drag(aero_obj, geometry_obj, design, propulsion_obj, W, state_input, airfoiltype)
               ...
               % Get CL_alpha
               % NOTE: CL_alpha_Raymer is not defined in this class or toolbox.
               % Pre-existing gap - use get_CL_alpha instead once wired.
               ...
               DragResults.CL_minD = aero_obj.compute_CL_minD(aero_obj.CL_alpha, aero_obj.alpha_L0);

               % Compute CD (done?) (double-check results later)
               aero_obj.K1 = aero_obj.compute_K1(state_input(1), geometry_obj.mainwings.AR, aero_obj.e_osw, geometry_obj.mainwings.LE_sweep);
               DragResults.CD_design = aero_obj.get_CD(DragResults.CD0_design, DragResults.CDi_design, aero_obj.CL, DragResults.CL_minD, airfoiltype, state_input, aero_obj.K1);
          ...
          function F = get_F(aero_obj, fuselage_diam, b)
               F = AeroLevel3.F(fuselage_diam, b);
          end
          ...
          % Estimate the dq searshaack thing
          ...
          % Estimate the drag factor due to the windmilling jet engine.
