...
%   and a field air-density ratio sigma = rho/rho_SL, it returns the
...
%   setting). Dividing by a separately-modeled alpha would double-count the
%   thrust model, so this class takes no prop object and applies no alpha. The
%   result is already on the same sea-level-static T_SL/W_TO axis the diagram
%   plots. See the class-header contrast note in TakeoffConstraint.m.
%
...
%   is the field air-density ratio (Example 4.2 uses 0.95 for a hot day near
%   sea level -- NOT an ISA AircraftState value, so it is an explicit input);
%   CLmax_TO is the flapped-takeoff maximum lift coefficient
%   (HighLiftConfigBridge.polar(aero,"takeoff_flaps_gear_down").CLmax); and
%   37.5 is the FAR-25 Takeoff Parameter calibration constant (Eq. 4.15,
%   English units, BFL in ft). Example 4.2 hand-check: BFL = 12,000 ft gives
%   TOP25 = 320.0 (Eq. 4.47), and with sigma = 0.95 the denominator constant
%   is 0.95 * 320 = 304.0 (Eq. 4.48), so T/W = (W/S)/(304.0 * CLmax_TO).
    ...
    properties (SetAccess = private)
        aero    % AerodynamicsBase -- supplies CLmax_TO via HighLiftConfigBridge.polar(aero,"takeoff_flaps_gear_down").CLmax
    ...
    methods
        ...
        function obj = TakeoffFieldLengthConstraint(name, aero, BFL_ft, sigma)
            arguments
                name   (1,1) string
        ...
        %   [Metabook Eqs. 4.15-4.16; worked Ex. 4.2 Eqs. 4.47-4.48.]
            CLmax_TO = HighLiftConfigBridge.polar(obj.aero, "takeoff_flaps_gear_down").CLmax;
            ...
            if ~isfinite(coeff)
                error('TakeoffFieldLengthConstraint:nonFiniteTerm', ...
                    ['Constraint "%s": Takeoff-Parameter T/W slope is non-finite ', ...
                     ...
                     '(HighLiftConfigBridge takeoff_flaps_gear_down).'], ...
    ...
    methods (Static)
        ...
        %   injected aero. Reads cond.BFL_ft and cond.sigma. prop is accepted
        %   for a uniform factory signature but IGNORED -- the TOP correlation
        %   models takeoff thrust statistically and needs no propulsion object
        %   (see class header "NO THRUST-LAPSE ALPHA AND NO PROP"). Uniform
        %   factory dispatched by ConstraintType.
            obj = TakeoffFieldLengthConstraint(string(cond.name), aero, ...
                cond.BFL_ft, cond.sigma);
        end
