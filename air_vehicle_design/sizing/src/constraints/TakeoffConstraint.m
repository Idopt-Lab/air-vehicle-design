...
%
%   CLmax_TO/CD0_TO are pulled fresh from the aero object each call, not
%   constraint inputs. get_CLmax_TO/get_Delta_CD0_TO are the flapped-takeoff
...
%   there); formalizing a high-lift-config argument is a deferred TODO
%   (ToDo_Darshan.md §1). Any AerodynamicsBase subclass passed here must
%   implement get_CLmax_TO/get_Delta_CD0_TO (see compute_C for the arity note).
%
%   EQUATION [Mattingly, Heiser, Pratt, "Aircraft Engine Design," 2nd ed.,
%   AIAA, 2002 -- the point-performance Master Equation specialized to the
%   ground-roll (takeoff) segment via T_SL >> (D+R) and dh/dt=0, which zero
%   the A and D terms and leave a B*(W_TO/S) term plus a constant C term
%   (drag/rolling-friction ground-roll correction). Matches
%   NPTEL_Fighter_Aircraft_Sizing.ipynb's Takeoff class (cells 32-33) for the
%   B term, and Brandt's F-16A.xls Consts-sheet takeoff row (row 32) for the
%   full equation including C. NOT Raymer: Raymer ch. 5 ("Takeoff Distance,"
%   pp. 128-130) gives only the graphical Takeoff Parameter / Fig. 5.4 method,
%   with no closed-form W/S-linear equation. Cross-checked against temp_Casey's
%   takeoff_constraint.m (reference only): its term1 is algebraically the B
%   term, its term2 = 0.7*CD0/(beta*CLmax) + mu is the C term. Reproduces
%   Brandt's takeoff T/W to within 0.5% at W/S=90 -- see
...
%
%   where rho is sea-level density; g is standard gravity, included (unlike
%   the NPTEL notebook's SI form) because this framework's T/W is weight-based
%   (English lbf/lbf), so mass density rho must be converted to a weight
...
%   values: examples/F16A/mds/f16a_requirements.md.
    ...
    methods
        ...
        %   size.
        %
        %   FAILS LOUDLY on a non-finite term: B and C are built from the aero
        %   flapped CLmax_TO/CD0_TO and the prop thrust lapse, either of which
        %   can be non-finite. A NaN required_TW is silently omitted from
        %   ConstraintAnalysis's max() envelope (reads as SATISFIED off a curve
        %   that does not exist), so error here instead -- the same protection
        %   every Master-Equation constraint gets. CLmax_TO, alpha, and the
        %   drag polar are fetched once per call and passed to
        %   compute_B/compute_C; the fetch stays inside the call so the
        %   constraint tracks the current sizing-loop iteration and fidelity.
    ...
    methods (Access = protected)
        ...
        %   flapped takeoff values. Takes both already-fetched (required_TW
        %   fetches them once). required_TW's CD0_TO fetch dispatches
        %   get_Delta_CD0_TO's non-uniform arity across fidelity levels
        %   (F16AeroL1/L2 take no argument, F16AeroL3 takes the flight state
        %   for a gear-strut Reynolds-number lookup) via metaclass reflection
        %   on aero's declared InputNames -- see get_Delta_CD0_TO_dispatched.
