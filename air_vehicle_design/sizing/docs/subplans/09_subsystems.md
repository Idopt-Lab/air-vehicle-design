...
gear-load-split decision is made (Raymer's 90/10, item 8). **Not yet implemented.** Two genuine gaps
remain with no textbook source found anywhere in-repo — item 7 (battery volumetric energy density)
and item 11 (gear bay-volume packaging) — planned to ship as documented-TODO / deliberately-failing
tests, matching the established convention (`TestWeightsL1.testTODO_RaymerTable61CoefficientsNotInRepo`),
not as a blocker to starting `io`/implementation on everything else.
...
- **L1** — tabulation only, no geometry: Nicolai Ch. 8 §8.1.10/8.1.11-style density/packaging-factor
  ...
  a plain `properties` STATE field on `F16WeightsL1` (`examples/F16A/F16WeightsL1.m:74`, "candidate
...
| File | Purpose |
|---|---|
| `examples/F16A/F16SubsystemsL1.m` | Tabulation inputs from `f16a_L1.json`; delegates to `SubsystemsL1` |
| `examples/F16A/F16SubsystemsL2.m` | Injected `F16GeomL2` + fuel-weight source (mission or `F16WeightsL2`); delegates to `SubsystemsL2` |
| `examples/F16A/F16SubsystemsL3.m` | Injected `F16GeomL3` + fuel-weight source; delegates to `SubsystemsL3` |
| `examples/F16A/F16LandingGearL2.m`, `F16LandingGearL3.m` | **F-16-only, no abstract `Base` tier** — Raymer 6th ed. Ch. 11 (p. 337) statistical tire sizing (Table 11.1, p. 344) off `W_TO`/gear-load-split, producing tire/strut size + a bay-volume estimate. Not every airframe has conventional landing gear (e.g. seaplanes), so this does not get a generic `src/disciplines/` home — it stays F-16-example-only, but still follows the toolbox-static-equations pattern where that makes sense. |
| `examples/F16A/F16Subsystems{L1,L2,L3}.md`, `F16LandingGear{L2,L3}.md` | Per-class companion docs (input/derived tables, per-method citations) |
...
- **Avionics-volume material across all reference-extract files:** checked all 31
  `temp_AI/docs/disciplines/reference_extracts/*.md` files (21 numbered Nicolai chapter extracts,
...
No T.O. 1F-16A-1 data was found anywhere in this repo (`temp_Casey`, `temp_AI`, or elsewhere) giving an
