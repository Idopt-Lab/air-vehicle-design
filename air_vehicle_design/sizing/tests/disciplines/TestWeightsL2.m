...
%     WeightsL1 (compute_We_roskam / We_roskam).  See TestWeightsL1 for those tests.
    ...
    % ------------------------------------------------------------------ %
    % Component weight estimates (Raymer Table 15.2 surface-density method)
    % ------------------------------------------------------------------ %
    ...
    methods (Test)
        ...
        function testFuselageWeightFormula(tc)
            % Fuselage: 4.8 lbf/ft² × S_wet_fus.  F-16 S_wet_fus = 750 ft² [estimate]
            % Expected: 4.8 × 750 = 3600 lbf  [Raymer Table 15.2]
            g = F16WeightsL2();
            W = g.weight_fuselage(31377);
            expected = 4.8 * g.S_wet_fus;
            fprintf('\n    L2 fuselage weight: %.0f lbf (expected %.0f lbf)\n', W, expected);
            tc.verifyEqual(W, expected, 'AbsTol', 1.0, ...
                'Fuselage weight must equal 4.8 lbf/ft² × S_wet_fus [Raymer Table 15.2].');
        end

        function testLandingGearWeightFormula(tc)
            % LG: 0.033 × W_TO for non-Navy fighter.  [AE481 metabook §7]
            % Expected: 0.033 × 31377 = 1035 lbf
            g = F16WeightsL2();
            W_TO = 31377;
            W    = g.weight_landing_gear(W_TO);
            expected = 0.033 * W_TO;
            fprintf('\n    L2 LG weight: %.0f lbf (expected %.0f lbf)\n', W, expected);
            tc.verifyEqual(W, expected, 'AbsTol', 1.0, ...
                'LG weight must equal 0.033 × W_TO for non-Navy fighter [AE481 metabook §7].');
        end
