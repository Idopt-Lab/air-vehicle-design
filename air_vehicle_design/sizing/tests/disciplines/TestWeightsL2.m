...
%                       + 180.00      = 15844.629507 lbf
%    (12) W_strake = wing density * S_exposed_strake = 9.0 * 20 = 180.00 lbf
%         (exact). Raymer Table 15.2 has no strake row, and the class takes
%         the WING row. Brandt's own coefficient is 4.5 psf, which gave
%         90.00 lbf until 2026-09-02, so this row is a 90 lbf deliberate
%         divergence from Brandt. The exposed area is INJECTED from
%         geom.S_exposed_strake. ADDED 2026-07-29, re-based 2026-09-02.
...
%   which is the comparison report's subject, not this file's.
    ...
    methods (Test)
        ...
        function testOEWGreaterThanZero(tc)
            w = TestWeightsL2.makeW2();
            tc.verifyGreaterThan(w.get_OEW(31377), 0, 'L2 OEW must be positive.');
        end
    ...
    methods (Test)
        ...
        function testWingWeightHandComputed(tc)
            ...
            w = TestWeightsL2.makeW2();
            tc.verifyEqual(w.W_wings, 1766.034621, 'AbsTol', 1e-4, ...
        ...
        function testFuselageWeightHandComputed(tc)
            ...
            w = TestWeightsL2.makeW2();
            tc.verifyEqual(w.W_fuselage, 3505.451136, 'AbsTol', 1e-4, ...
        ...
        function testLandingGearWeightHandComputed(tc)
            ...
            w = TestWeightsL2.makeW2();
            tc.verifyEqual(WeightsL2.compute_weight_landing_gear(w.aircraft_category, 31377), 1035.441, 'AbsTol', 1e-6, ...
        ...
        function testAllElseEmptyHandComputed(tc)
            ...
            w = TestWeightsL2.makeW2();
            tc.verifyEqual(WeightsL2.compute_weight_all_else_empty(w.aircraft_category, 31377), 5334.09, ...
                'AbsTol', 1e-6, ...
        ...
        %   in any one component, coefficient or injected area breaks it.
            w = TestWeightsL2.makeW2();
            tc.verifyEqual(w.get_OEW(31377), 15844.629507, 'RelTol', 1e-5, ...
                ['L2 OEW(31377) must equal the hand-summed Table 15.2 psf + ' ...
                 'fraction sub-table + strake buildup = 15844.629507 lbf. Was ' ...
                 '15754.629507 while the strake took Brandt 4.5 psf; it now takes ' ...
                 'the WING density, 9.0 psf on 20 ft^2 = 180.00 lbf.']);
        end
        ...
        %   the report and the tests above exercise.
            w = TestWeightsL2.makeW2();
            W_TO  = 31377;
            cat   = w.aircraft_category;
            W_t   = w.W_tail;
            total = w.W_wings + W_t.HT + W_t.VT + w.W_fuselage ...
                    + WeightsL2.compute_weight_landing_gear(cat, W_TO) ...
                    + WeightsL2.compute_weight_installed_engine(cat, w.N_en, w.W_en) ...
                    + WeightsL2.compute_weight_all_else_empty(cat, W_TO) + w.W_strake;
    ...
    methods (Test)
        ...
        %   difference is exactly (0.17+0.033)*(W2-W1); header row (11).
            ...
            tc.verifyEqual(w.get_OEW(45000) - w.get_OEW(31377), delta_expected, ...
                'AbsTol', 1e-6, ...
    ...
    methods (Test)
        ...
        %   docs/weights_parameter_usage.md §2.
            ...
            tc.verifyEqual(w.S_exposed_planform_wing,  geom.S_exposed_wing, 'AbsTol', 1e-9, ...
                'S_exposed_planform_wing must be geom.S_exposed_wing.');
            tc.verifyEqual(w.S_exposed_planform_ht, geom.S_exposed_ht, 'AbsTol', 1e-9, ...
                'S_exposed_planform_ht must be geom.S_exposed_ht.');
            tc.verifyEqual(w.S_exposed_planform_vt, geom.S_exposed_vt, 'AbsTol', 1e-9, ...
                'S_exposed_planform_vt must be geom.S_exposed_vt.');
            tc.verifyNotEqual(w.S_exposed_planform_ht, geom.S_ht, ...
                'S_exposed_planform_ht must NOT be geom.S_ht (the FULL 108 ft^2 planform).');
            tc.verifyNotEqual(w.S_exposed_planform_vt, geom.S_vt, ...
                'S_exposed_planform_vt must NOT be geom.S_vt (the FULL 60 ft^2 planform).');
        ...
        %   for that reason -- must appear nowhere.
            ...
            oew_official = w.get_OEW(31377);
            tc.verifyLessThan(abs(oew_official - 15844.629507), 1e-3 * 15844.629507, ...
    ...
    methods (Test)
        ...
        function testAllFourteenDependentPropertiesExist(tc)
        %TESTALLFOURTEENDEPENDENTPROPERTIESEXIST  The declared INPUT/DERIVED split.
        %   A count change means the split moved and the sweeps below no longer
        %   cover what they claim to. 14 as of 2026-09-02: the five injected
        %   areas now include S_exposed_planform_strakes, because the strake
        %   area moved out of .weights and into the geometry.
            names = TestWeightsL2.dependentNames();
            tc.verifyEqual(numel(names), 14, ...
                'F16WeightsL2 must declare exactly 14 Dependent properties.');
        end
        ...
        %   14 Dependents (metaclass-derived, so a newly added one is covered
        ...
        %   every field of the W_tail struct, to be finite.
            ...
            tc.verifyEqual(numel(vals), 15, ...
                'Sweep must cover 14 Dependents = 15 scalars (W_tail has HT and VT).');
        end
    ...
    methods (Test)
        ...
        function testWingUnitWeightAllCategories(tc)
            ...
            tc.verifyEqual(WeightsL2.compute_weight_wing('jet_fighter', 1), 9.0, ...
                'AbsTol', 1e-6, 'jet_fighter wing psf must be 9.0 [metabook_data.md:321].');
            tc.verifyEqual(WeightsL2.compute_weight_wing('jet_transport', 1), 10.0, ...
                'AbsTol', 1e-6, 'jet_transport wing psf must be 10.0 [metabook_data.md:321].');
            tc.verifyEqual(WeightsL2.compute_weight_wing('general_aviation', 1), 2.5, ...
        ...
        function testTailUnitWeightsAllCategories(tc)
            ...
            tc.verifyEqual(WeightsL2.compute_weight_HT('jet_fighter', 1), 4.0, ...
                'AbsTol', 1e-6, 'jet_fighter HT psf must be 4.0 [metabook_data.md:322].');
            tc.verifyEqual(WeightsL2.compute_weight_HT('jet_transport', 1), 5.5, ...
                'AbsTol', 1e-6, 'jet_transport HT psf must be 5.5 [metabook_data.md:322].');
            tc.verifyEqual(WeightsL2.compute_weight_HT('general_aviation', 1), 2.0, ...
                'AbsTol', 1e-6, 'general_aviation HT psf must be 2.0 [metabook_data.md:322].');
            tc.verifyEqual(WeightsL2.compute_weight_VT('jet_fighter', 1), 5.3, ...
                'AbsTol', 1e-6, 'jet_fighter VT psf must be 5.3 [metabook_data.md:323].');
            tc.verifyEqual(WeightsL2.compute_weight_VT('jet_transport', 1), 5.5, ...
                'AbsTol', 1e-6, 'jet_transport VT psf must be 5.5 [metabook_data.md:323].');
            tc.verifyEqual(WeightsL2.compute_weight_VT('general_aviation', 1), 2.0, ...
        ...
        function testFuselageUnitWeightAllCategories(tc)
            ...
            tc.verifyEqual(WeightsL2.compute_weight_fuselage('jet_fighter', 1), 4.8, ...
                'AbsTol', 1e-6, 'jet_fighter fuselage psf must be 4.8 [metabook_data.md:324].');
            tc.verifyEqual(WeightsL2.compute_weight_fuselage('jet_transport', 1), 5.0, ...
                'AbsTol', 1e-6, 'jet_transport fuselage psf must be 5.0 [metabook_data.md:324].');
            tc.verifyEqual(WeightsL2.compute_weight_fuselage('general_aviation', 1), 1.4, ...
        ...
        function testLookupUnknownCategoryErrors(tc)
            % Error contract on every lookup. The density and fraction
            % lookups are private, so each is reached through its public
            % compute_* with a unit area.
            tc.verifyError(@() WeightsL2.compute_weight_wing('spaceship', 1), ...
                'WeightsL2:UnknownCategory', 'compute_weight_wing must reject unknown categories.');
            tc.verifyError(@() WeightsL2.compute_weight_HT('spaceship', 1), ...
                'WeightsL2:UnknownCategory', 'compute_weight_HT must reject unknown categories.');
            tc.verifyError(@() WeightsL2.compute_weight_VT('spaceship', 1), ...
                'WeightsL2:UnknownCategory', 'compute_weight_VT must reject unknown categories.');
            tc.verifyError(@() WeightsL2.compute_weight_fuselage('spaceship', 1), ...
                'WeightsL2:UnknownCategory', 'compute_weight_fuselage must reject unknown categories.');
            tc.verifyError(@() WeightsL2.compute_weight_landing_gear('spaceship', 1), ...
                'WeightsL2:UnknownCategory', 'compute_weight_landing_gear must reject unknown categories.');
        end
        ...
        %   fall back to a default psf row.
            ...
            tc.verifyError(@() w.get_OEW(31377), 'WeightsL2:UnknownCategory', ...
