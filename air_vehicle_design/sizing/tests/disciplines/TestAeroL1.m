classdef TestAeroL1 < matlab.unittest.TestCase
     ...
     % These properties are for some of the "brandt gap" tests, so we don't
     % have to repeatedly declare them.
     properties (Constant)
          ALT_FT = 36000
          M_SUB  = 0.6
          M_SUP  = 1.6
     ...
     methods (Test)
          ...
          % ================================================================== %
          % BRANDT GAP TESTS -- every L1 aero value that Brandt also computes.
          %
          ...
          %
          % Condition: 36,000 ft, M = 0.6 subsonic and M = 1.6 supersonic. The
          % 0.95 to 1.05 band is skipped because drag_polar returns NaN there.
          % Brandt's CDmin_sub is his SKIN-FRICTION basis, which is the same
          % basis the framework CD0 uses. His mission CD0 folds in form,
          % interference and wave drag, so it is a different quantity.
          % ================================================================== %


          % This next function was a stencil (or "cookiecutter") for the subsequent tests in
          % this section.
          function testK1_manually(tc)
               M = 0.1; % This should be a sweep across all Mach numbers in Brandt's "aero" sheet.

               % Initialize some objects for Brandt classes.
               BrandtGeomObj = BrandtGeometry;
               BrandtGeomObj.analyze;
               BrandtAeroObj = BrandtAerodynamics(BrandtGeomObj);
               BrandtAeroObj.analyze;
               expected = BrandtAeroObj.run(M).K1;

               % Get dependencies for computing K1 at L1 aero (AR,
               % Lambda_LE_deg, M).
               % Obtain AR from L1 disciplines and json inputs.
               % Obtain Lambda_LE_deg (main wing sweep)(degrees) from L1
               % disciplines and json inputs.
               aeroL1        = F16AeroL1(f16a_spec_path(1));
               AR            = aeroL1.AR;
               Lambda_LE_deg = aeroL1.Lambda_LE_deg;

               % Get the computed/expected value.
               computed = AeroL1.k1_from_geometry(AR, Lambda_LE_deg, M);

               % This next part was written by Claude. you can tell because
               % it's excessively verbose.
               fprintf(['\n    K1 at M=%.2f (AR=%.4f, Lambda_LE=%.4f deg):\n' ...
                    '      computed (Raymer Eq. 12.49/12.50) = %.10f\n' ...
                    '      expected (Brandt Aero!G12/G10)    = %.10f\n' ...
                    '      difference                        = %+.4f %%\n'], ...
                    M, AR, Lambda_LE_deg, computed, expected, ...
                    100*(computed - expected)/expected);
               tc.verifyEqual(computed, expected, 'RelTol', 1e-2, ...
                    ['Raymer Eq. 12.49/12.50 K1 and Brandt Aero!G12/G10 K1 must ' ...
                    'agree within 1% at subsonic Mach (F16AeroL1.m class header). ' ...
                    'A red here means an input or an Oswald equation moved -- ' ...
                    'find which side, do not widen the tolerance.']);
          end
          ...
          function testBrandtGapCD0Subsonic(tc)
               a = TestAeroL1.makeAero(); % <- Helper function so I don't have to re-initialize objects every time
               b = TestAeroL1.brandtAero();
               computed = a.drag_polar(AircraftState(tc.ALT_FT, tc.M_SUB)).CD0;
               tc.verifyGap('CD0 subsonic', computed, b.CDmin_sub, 0.03);
          end

          function testBrandtGapCD0Supersonic(tc)
               a = TestAeroL1.makeAero();
               b = TestAeroL1.brandtAero();
               computed = a.drag_polar(AircraftState(tc.ALT_FT, tc.M_SUP)).CD0;
               tc.verifyGap('CD0 supersonic', computed, b.run(tc.M_SUP).CD0, 0.35);
          end
          ...
          function testBrandtGapK2Subsonic(tc)
               % L1 models the fighter as uncambered, so K2 is 0 exactly. Brandt
               % carries a small negative camber term. A relative bound cannot
               % express that pair, so this one is absolute.
               a = TestAeroL1.makeAero();
               b = TestAeroL1.brandtAero();
               computed = a.drag_polar(AircraftState(tc.ALT_FT, tc.M_SUB)).K2;
               tc.verifyGapAbs('K2 subsonic', computed, b.run(tc.M_SUB).K2, 7e-3);
          end
          ...
          function testBrandtGapCLmaxClean(tc)
               a = TestAeroL1.makeAero();
               b = TestAeroL1.brandtAero();
               tc.verifyGap('CLmax clean', a.get_CLmax([]), b.CLmax_clean, 0.55);
          end

          function testBrandtGapCLmaxTakeoff(tc)
               a = TestAeroL1.makeAero();
               b = TestAeroL1.brandtAero();
               tc.verifyGap('CLmax takeoff', a.get_CLmax_TO(), b.CLmax_takeoff, 0.35);
          end

          function testBrandtGapCLmaxLanding(tc)
               a = TestAeroL1.makeAero();
               b = TestAeroL1.brandtAero();
               tc.verifyGap('CLmax landing', a.get_CLmax_L(), b.CLmax_landing, 0.50);
          end
          ...
          function testBrandtGapCD0Takeoff(tc)
               ...
               computed = a.drag_polar(AircraftState(tc.ALT_FT, tc.M_SUB)).CD0 ...
                    + a.get_Delta_CD0_TO();
               tc.verifyGap('CD0 takeoff', computed, b.CD0_takeoff, 0.03);
          end
