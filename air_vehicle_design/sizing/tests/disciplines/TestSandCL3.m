...
%   "never self-referential" rule).
    ...
    methods (Test)
        ...
        % group_weight switch.
            ...
            tc.verifyEqual(s3.x_cg, expected, 'RelTol', 1e-9, ...
        ...
        function testDeltaXAcBelow04IsZero(tc)
            tc.verifyEqual(SandCL3.delta_x_ac(0.3), 0, 'AbsTol', 1e-12);
        end
        ...
        % M = 1.5 (> 1.1): 0.112 - 0.004*1.5 = 0.112 - 0.006 = 0.106.
            ...
            tc.verifyEqual(received, 0.106, 'AbsTol', 1e-12);
        end
        ...
        % supersonic branch's value at the same M.
            tc.verifyEqual(SandCL3.delta_x_ac(0.4), 0, 'AbsTol', 1e-12, ...
        ...
        % x_ac_wing must equal the bare quarter-MAC point 13.566001.
            ...
            tc.verifyEqual(received, 13.566001197, 'AbsTol', 1e-6);
        end
        ...
        %   val = 0.02*6^2*40/(5*200) = 0.02*36*40/1000 = 28.8/1000 = 0.0288
            ...
            tc.verifyEqual(received, 0.0288, 'AbsTol', 1e-12);
        end
        ...
        % per_deg, not an approximation and not the reverse ratio.
            ...
            tc.verifyEqual(per_rad, per_deg * (180/pi), 'AbsTol', 1e-12, ...
                'Cm_alpha_fus_per_rad must equal per_deg * (180/pi) exactly.');
            tc.verifyEqual(per_rad / per_deg, 57.29577951308232, 'AbsTol', 1e-9, ...
        ...
        %       = 0.4 + 0.1 - 0.54*2.9 = 0.5 - 1.566 = -1.066
            ...
            tc.verifyEqual(received, -1.066, 'AbsTol', 1e-9);
        end
        ...
        %       = 0.4 + 0.1 - 1.566 + 0.04*1.1 = 0.5 - 1.566 + 0.044 = -1.022
            ...
            tc.verifyEqual(received, -1.022, 'AbsTol', 1e-9);
        ...
        function testStaticMarginHandComputed(tc)
            tc.verifyEqual(SandCL3.static_margin(2.5, 2.3), 0.2, 'AbsTol', 1e-12);
        end
        ...
        % loudly (received 0.002 != expected 0.2, not a subtle RelTol miss).
            ...
            tc.verifyEqual(received, 0.2, 'AbsTol', 1e-12);
        ...
        %   CLw = 5 * deg2rad(2+1-(-2)) = 5*deg2rad(5) = 5*0.08726646 = 0.4363323
            ...
            tc.verifyEqual(received, expected, 'AbsTol', 1e-9);
            tc.verifyEqual(received, 0.436332313, 'AbsTol', 1e-6);
        end
        ...
        %   CLh = 4 * deg2rad(4) = 4*0.06981317 = 0.27925268
            ...
            tc.verifyEqual(received, expected, 'AbsTol', 1e-9);
            tc.verifyEqual(received, 0.279252680, 'AbsTol', 1e-6);
        end
        ...
        %   val = -0.595932 * 10 = -5.95932
            ...
            tc.verifyEqual(received, -5.95932, 'AbsTol', 1e-6);
        end
        ...
        %          = -0.01 * 1.76047227 / 4.53208889 = -0.0038844610...
            ...
            tc.verifyEqual(received, -0.003884461030422876, 'AbsTol', 1e-9);
        end
        ...
        % of AR/sweep -- the formula is a pure multiplicative scaling.
            ...
            tc.verifyEqual(received, 0, 'AbsTol', 1e-12);
        end
        ...
        %   Cmcg' = 0.2 - 0.02 + 0.05 - 0.27 - 0 + 0 = -0.04
            ...
            tc.verifyEqual(received, -0.04, 'AbsTol', 1e-9);
        ...
        % apex/sweep/MAC wiring bug (which would produce a much larger gap).
            ...
            s3 = F16SandCL3(f16a_spec_path(3), g3, w3, a3, prop);
            tc.verifyEqual(s3.x_acw, 25.589, 'RelTol', 0.08, ...
            ...
            tc.verifyGreaterThan(s3.x_acw, g3.x_apex_wing, ...
        ...
        function testF16SandCL3XAchAftOfHtLeadingEdge(tc)
            [g3, w3, a3, prop] = TestSandCL3.makeF16Objects();
            s3 = F16SandCL3(f16a_spec_path(3), g3, w3, a3, prop);
            tc.verifyTrue(isfinite(s3.x_ach));
            tc.verifyGreaterThan(s3.x_ach, g3.x_le_ht, ...
        ...
        % for a real airframe, so Cm_alpha_fus must be strictly positive.
            [g3, w3, a3, prop] = TestSandCL3.makeF16Objects();
            s3 = F16SandCL3(f16a_spec_path(3), g3, w3, a3, prop);
            tc.verifyTrue(isfinite(s3.Cm_alpha_fus));
            tc.verifyGreaterThan(s3.Cm_alpha_fus, 0);
        end

        function testF16SandCL3XNpFiniteAndWithinFuselage(tc)
            [g3, w3, a3, prop] = TestSandCL3.makeF16Objects();
            s3 = F16SandCL3(f16a_spec_path(3), g3, w3, a3, prop);
            tc.verifyTrue(isfinite(s3.x_np));
            tc.verifyGreaterThan(s3.x_np, 0);
            tc.verifyLessThan(s3.x_np, g3.L_fus, ...
        ...
        % -- never backfilled into this unit test).
            [g3, w3, a3, prop] = TestSandCL3.makeF16Objects();
            s3 = F16SandCL3(f16a_spec_path(3), g3, w3, a3, prop);
            tc.verifyTrue(isfinite(s3.SM));
            tc.verifyGreaterThan(s3.SM, -0.5);
            tc.verifyLessThan(s3.SM, 0.5);
        end
        ...
        % would very likely disagree.
            [g3, w3, a3, prop] = TestSandCL3.makeF16Objects();
            s3 = F16SandCL3(f16a_spec_path(3), g3, w3, a3, prop);
            tc.verifyEqual(s3.Cm_alpha, s3.Cm_alpha_via_neutral_point(), 'RelTol', 1e-9, ...
        ...
        % for ANY delta_e -- a real answer for THIS airframe, not a gap.
            [g3, w3, a3, prop] = TestSandCL3.makeF16Objects();
            s3 = F16SandCL3(f16a_spec_path(3), g3, w3, a3, prop);
            tc.verifyEqual(s3.Delta_alpha_L0(5), 0, 'AbsTol', 1e-12);
            tc.verifyEqual(s3.Delta_alpha_L0(-12.3), 0, 'AbsTol', 1e-12);
        end
        ...
        function testConstructorRequiresAllFiveArgs(tc)
            tc.verifyError(@() F16SandCL3(), 'MATLAB:minrhs');
            tc.verifyError(@() F16SandCL3(f16a_spec_path(3)), 'MATLAB:minrhs');
        end
        ...
        % Mirrors TestGeomL2's testDerivedPropertiesAreReadOnly template.
            [g3, w3, a3, prop] = TestSandCL3.makeF16Objects();
            s3 = F16SandCL3(f16a_spec_path(3), g3, w3, a3, prop);
            tc.verifyError(@() setfield(s3, 'x_cg', 1), 'MATLAB:class:noSetMethod'); %#ok<SFLD>
            tc.verifyError(@() setfield(s3, 'x_acw', 1), 'MATLAB:class:noSetMethod'); %#ok<SFLD>
            tc.verifyError(@() setfield(s3, 'x_ach', 1), 'MATLAB:class:noSetMethod'); %#ok<SFLD>
            tc.verifyError(@() setfield(s3, 'x_np', 1), 'MATLAB:class:noSetMethod'); %#ok<SFLD>
            tc.verifyError(@() setfield(s3, 'SM', 1), 'MATLAB:class:noSetMethod'); %#ok<SFLD>
            tc.verifyError(@() setfield(s3, 'Cm_alpha', 1), 'MATLAB:class:noSetMethod'); %#ok<SFLD>
        end
        ...
        % reconstruction of the object.
            ...
            tc.verifyEqual(s3.x_acw, v0 + 5, 'AbsTol', 1e-9, ...
        ...
        function testIsaChecks(tc)
            [g3, w3, a3, prop] = TestSandCL3.makeF16Objects();
            s3 = F16SandCL3(f16a_spec_path(3), g3, w3, a3, prop);
            tc.verifyTrue(isa(s3, 'StabControlBase'));
            tc.verifyTrue(isa(s3, 'SandCModelL3'));
        ...
        %   by calling CL_w a second time.
            ...
            tc.verifyEqual(s3.CL_w(5), expected, 'RelTol', 1e-10, ...
        ...
        %   independently recomputed from the real CL_alpha_tail.
            ...
            tc.verifyEqual(s3.CL_h(5, -2), expected, 'RelTol', 1e-10, ...
                'CL_h must equal CL_alpha_tail*deg2rad(alpha+i_h-epsilon-alpha_0Lh).');
            tc.verifyError(@() s3.CL_h(5), 'MATLAB:minrhs', ...
        ...
        function testF16SandCL3CmCgTrimReturnsNaNUntilCm0AirfoilWingSupplied(tc)
            ...
            tc.verifyError(@() s3.Cm_cg_trim(5), 'MATLAB:minrhs', ...
