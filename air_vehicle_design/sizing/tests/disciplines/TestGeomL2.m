...
%       live instead of carrying Brandt's frozen outputs.
    ...
    methods (Test)
        ...
        % Migrated (same hand-computed expected values) from the now-retired
        % tests/disciplines/TestTailL2.m -- repointed from TailL2.*/
        % F16TailL2(geom) to GeomL2.*/a real F16GeomL2(...) object's own
        % C_HT_nicolai/C_VT_nicolai properties and size_tail_nicolai() method
        % (the old test constructed F16TailL2(geom) as a SEPARATE object
        % injected with geom; the new equivalent calls .size_tail_nicolai()
        % directly ON the geometry object itself).
        %
        % METHOD -- same governing identity as GeomL1's Raymer 7th ed.
        % Table 6.4 form, confirmed algebraically identical to Nicolai &
        % Carichner's own Eq. (11.1)/(11.2) [temp_AI/docs/disciplines/
        % reference_extracts/11_tail_sizing.md Secs. 11.2-11.3, pp. 286, 289]:
        %   C_VT = (l_VT*S_VT)/(b*S_ref)      ==>  S_VT = C_VT*b*S_ref/l_VT
        %   C_HT = (l_HT*S_HT)/(cbar*S_ref)   ==>  S_HT = C_HT*cbar*S_ref/l_HT
        % COEFFICIENTS: Nicolai & Carichner Table 11.6, "General Dynamics
        % F-16" row, p.289: C_HT=0.3, C_VT=0.094.
        %
        % NOTE: the old testConstructorRejectsWrongGeomTier test (which
        % asserted F16TailL2(g1) errors because g1 is an L1 geometry object)
        % has NO direct equivalent here -- there is no separate constructor
        % accepting an injected geometry object any more, so the tier-mismatch
        % class of bug that test guarded against can no longer occur (tail
        % sizing is a method ON the correctly-typed geometry object itself,
        % not a separate object type-checked against it). Deliberately
        % dropped, not migrated.
        % ==================================================================================================================================== %

        function testNicolaiComputeSHTFormula(tc)
        ...
        %   19*37=703, remainder 9.5, 9.5/19=0.5 exactly -> 37.5 EXACTLY.
            val = GeomL2.compute_S_HT(0.3, 9.5, 250, 19);
            tc.verifyEqual(val, 37.5, 'AbsTol', 1e-9);
        end

        function testNicolaiComputeSVTFormula(tc)
        ...
        %   (19*34=646, remainder 12, 12/19=0.631578947368421... repeating).
            val = GeomL2.compute_S_VT(0.094, 28, 250, 19);
            tc.verifyEqual(val, 34.631578947368421, 'RelTol', 1e-12);
        end

        function testNicolaiComputeSHTRejectsNonPositiveDenominator(tc)
            tc.verifyError(@() GeomL2.compute_S_HT(0.3, 9, 250, 0), 'MATLAB:validators:mustBePositive');
        end

        function testNicolaiComputeSVTRejectsNonPositiveDenominator(tc)
            tc.verifyError(@() GeomL2.compute_S_VT(0.094, 28, 250, 0), 'MATLAB:validators:mustBePositive');
        end

        function testNicolaiTailArmReusesGeomL1Formula(tc)
        % GeomL2 does NOT duplicate the tail-arm formula -- confirms the
        % cross-toolbox call target (GeomL1.compute_tail_arm) still gives
        ...
        function testSizeTailNicolaiReadsObjLiveAndMatchesHandComputedFormula(tc)
        % A plain struct suffices as `obj` -- GeomL2.size_tail_nicolai only
        % reads obj.C_HT_nicolai/obj.C_VT_nicolai/obj.S_ref/obj.b_wing/
        % obj.cbar_wing/obj.L_fus via dot access, which structs support.
        %   C_HT_nicolai=0.3, C_VT_nicolai=0.094, S_ref=250, b=28, cbar=9.5, L_fus=40
        %   L_HT=L_VT=0.475*40=19
        %   S_ht = 0.3*9.5*250/19  = 37.5 EXACTLY (see testNicolaiComputeSHTFormula)
        %   S_vt = 0.094*28*250/19 = 34.631578947368421... (see testNicolaiComputeSVTFormula)
            obj = struct('C_HT_nicolai', 0.3, 'C_VT_nicolai', 0.094, ...
                'S_ref', 250, 'b_wing', 28, 'cbar_wing', 9.5, 'L_fus', 40);
            result = GeomL2.size_tail_nicolai(obj);
            fprintf('\n    GeomL2.size_tail_nicolai: S_ht received=%.9f expected=37.500000000 | S_vt received=%.9f expected=34.631578947\n', ...
        ...
        function testSizeTailNicolaiResultFieldNamesAreLowercase(tc)
            obj = struct('C_HT_nicolai', 0.3, 'C_VT_nicolai', 0.094, ...
                'S_ref', 300, 'b_wing', 30, 'cbar_wing', 11, 'L_fus', 46.5);
            result = GeomL2.size_tail_nicolai(obj);
            tc.verifyTrue(isfield(result, 'S_ht'));
            tc.verifyTrue(isfield(result, 'S_vt'));
        end

        function testF16GeomL2NicolaiCoefficients(tc)
        % [Nicolai & Carichner Table 11.6, "General Dynamics F-16" row, p.289]
            g = F16GeomL2(f16a_spec_path(2), F16PropL2(f16a_spec_path(2)));
            tc.verifyEqual(g.C_HT_nicolai, 0.3, 'AbsTol', 0);
            tc.verifyEqual(g.C_VT_nicolai, 0.094, 'AbsTol', 0);
        end

        function testF16GeomL2SizeTailNicolaiAgainstOwnWingGeometry(tc)
        % Feeds F16GeomL2.size_tail_nicolai with its OWN live geometry --
        % self-referencing, no separate injected object.
        %
        % HAND-COMPUTED EXPECTED VALUES (same geometry fraction arithmetic
        % as the L1 case: S_ref=300, b=30, cbar=2729080/241081=
        ...
        %   S_vt = 0.094*30*300/22.0875 = 22560/589 = 38.302207130... ft^2
            g      = F16GeomL2(f16a_spec_path(2), F16PropL2(f16a_spec_path(2)));
            result = g.size_tail_nicolai();

            expected_S_ht = 46.126364801;
            expected_S_vt = 38.302207130;

            fprintf(['\n    F16GeomL2.size_tail_nicolai (own live geometry): ' ...
            ...
            tc.verifyEqual(result.S_ht, expected_S_ht, 'RelTol', 1e-6, ...
                'F16GeomL2.size_tail_nicolai S_ht must match the hand-computed Nicolai/Carichner formula result.');
            tc.verifyEqual(result.S_vt, expected_S_vt, 'RelTol', 1e-6, ...
                'F16GeomL2.size_tail_nicolai S_vt must match the hand-computed Nicolai/Carichner formula result.');
        end

        function testSizeTailNicolaiDoesNotSelfMutate(tc)
        % size_tail_nicolai is deliberately a NON-mutating query/comparison
        % method (contrast size_tail, which DOES self-mutate) -- it must
        % never clobber the primary size_tail path's S_ht/S_vt output.
            g       = F16GeomL2(f16a_spec_path(2), F16PropL2(f16a_spec_path(2)));
            S_ht0   = g.S_ht;
            S_vt0   = g.S_vt;
            g.size_tail_nicolai();
            tc.verifyEqual(g.S_ht, S_ht0, 'AbsTol', 0, ...
                'size_tail_nicolai must NOT mutate obj.S_ht.');
            tc.verifyEqual(g.S_vt, S_vt0, 'AbsTol', 0, ...
                'size_tail_nicolai must NOT mutate obj.S_vt.');
        end

        function testSizeTailNicolaiTracksGeometryMutationLive(tc)
        % LIVE-RECOMPUTE GUARD (parallels testWettedAreasLiveOnRead for the
        % optimization-ready property design): mutating AR_wing in place must
        % change the NEXT size_tail_nicolai() call's S_ht/S_vt with NO
        % reconstruction of the geometry object.
            g    = F16GeomL2(f16a_spec_path(2), F16PropL2(f16a_spec_path(2)));
            r0   = g.size_tail_nicolai();
            g.AR_wing = g.AR_wing + 1;   % optimizer-style in-place mutation
            r1   = g.size_tail_nicolai();
            ...
            tc.verifyNotEqual(r1.S_ht, r0.S_ht, ...
                'S_ht must change after mutating AR_wing -- no cached copy allowed.');
            tc.verifyNotEqual(r1.S_vt, r0.S_vt, ...
                'S_vt must change after mutating AR_wing -- no cached copy allowed.');
        end

        function testF16GeomL2NicolaiTailAreasArePositive(tc)
            g = F16GeomL2(f16a_spec_path(2), F16PropL2(f16a_spec_path(2)));
            result = g.size_tail_nicolai();
        ...
        function testF16GeomL2SizeTailSelfMutatesSHtSVt(tc)
        % PRIMARY, production path: size_tail() must self-mutate obj.S_ht/
        % obj.S_vt (this REPLACES what SizingLoopL2 used to do externally by
        % assigning geom.S_ht = tail_result.S_ht after calling a separate
        % injected tail object).
            g = F16GeomL2(f16a_spec_path(2), F16PropL2(f16a_spec_path(2)));
            result = g.size_tail();
            tc.verifyEqual(g.S_ht, result.S_ht, 'AbsTol', 0, ...
                'size_tail() must self-mutate obj.S_ht to match its own returned result.');
            tc.verifyEqual(g.S_vt, result.S_vt, 'AbsTol', 0, ...
                'size_tail() must self-mutate obj.S_vt to match its own returned result.');
        end

        % ==================================================================================================================================== %

        % ======================= CONTROL SURFACE SIZING (absorbed from the former src/sizing/ControlSurfaceSizer.m, 2026-08-03) ============= %
        % Migrated from the now-retired tests/sizing/TestControlSurfaceSizer.m
        % -- repointed from ControlSurfaceSizer(...)/.size(geom) to
        % GeomL2.compute_control_surface_areas(...) (low-level, arbitrary-
        % scalar tests) and a real F16GeomL2(...) object's OWN default
        % fraction properties (0.20/0.40/0/0/0.30/0.90) + .size_control_
        % surfaces() (high-level, F-16-specific test) -- same expected
        % numeric results as before, since the default fraction VALUES are
        % identical to the old ControlSurfaceSizer constructor call's.
        %
        % METHOD [Raymer, "Aircraft Design: A Conceptual Approach," 6th ed.,
        % AIAA, 2018, p.161 (Fig. 6.3, aileron), p.162 (Table 6.5, elevator/
        % rudder; constant-percent-chord control surfaces, Fig. 6.4)].
        % ==================================================================================================================================== %

        function testControlSurfaceAreasMatchHandComputedFormula(tc)
        % Arbitrary invented scalars (not F-16 data).
            ...
            result = GeomL2.compute_control_surface_areas(c_ail, b_ail, c_elev, b_elev, c_rud, b_rud, S_ref, S_ht, S_vt);
        ...
        function testControlSurfaceAreasResultFieldNamesMatchGeomPropertyCasing(tc)
            result = GeomL2.compute_control_surface_areas(0.2, 0.4, 0.3, 0.9, 0.3, 0.9, 300, 108, 60);
            tc.verifyTrue(isfield(result, 'S_ail'));
            tc.verifyTrue(isfield(result, 'S_elev'));
            tc.verifyTrue(isfield(result, 'S_rud'));
        end

        function testF16GeomL2SizeControlSurfacesUsesOwnDefaultFractions(tc)
        % F16GeomL2's own default fraction properties (0.20/0.40/0/0/0.30/
        % 0.90) drive size_control_surfaces(), rather than fractions passed
        % into a separate constructor -- same expected numeric results as
        % the old ControlSurfaceSizer(0.20, 0.40, 0, 0, 0.30, 0.90) call.
            g      = F16GeomL2(f16a_spec_path(2), F16PropL2(f16a_spec_path(2)));
