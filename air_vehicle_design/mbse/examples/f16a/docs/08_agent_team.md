...
|---|---|---|---|
| 1 | Can a stereotype property be an **enumeration**? | **Yes** — `addProperty(…, Type="MyEnumClass")` accepts both an `int32`-derived enumeration and a plain MATLAB `enumeration` class, as long as the classdef is on the path | `DataProvenance` and `SourceKind` can be real enumerations (Property Inspector dropdowns) instead of free strings |
