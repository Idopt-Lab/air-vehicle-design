function ranked = F16AEngineTradeStudy()
%F16AENGINETRADESTUDY Choose the F-16A engine and write the choice into the model.
%   RANKED = F16AENGINETRADESTUDY() scores every choice of the Engine variant on
%   thrust, mass and maturity, picks the best, and records that decision in the
%   Physical model, the Logical model and REQ_F16A_L01.
%
...
%   F16AAirframeTradeStudy and F16AFlightControlsTradeStudy; F16APhysicalTradeStudy
%   runs all three. Nothing here is shared with them on purpose: this file is
%   meant to be read straight through without opening another one.
%
...
% file is mechanics.
% ---------------------------------------------------------------------
VARIANTPATH = "F16A_Physical/Aircraft/Propulsion/Engine";  % the P variant holding the candidates
LOGICALROLE = "PropulsionSystem";                          % the L variant holding the kinds
DECISIONREQ = "REQ_F16A_L01";                              % where the decision is recorded
STEREOTYPE  = "EngineCandidate";                           % the properties this trade reads
...
% (TRL-1)/8 tops out at 1.0. The two RATIO criteria have no ceiling, which STEP 3
% checks for and reports (D-035).
crit = struct( ...
    "Name",   {"Thrust_SL_lb",  "Mass_lb",       "TRL"}, ...
    "Weight", {0.30,            0.35,            0.35}, ...
    "Value",  {@(x,b) x/b,      @(x,b) b/x,      @(x,b) (x-1)/8}, ...
    "Rule",   {"T/T_baseline",  "M_baseline/M",  "(TRL-1)/8"}, ...
    "Better", {"more thrust",   "less mass",     "more mature"});
nCrit = numel(crit);
...
fprintf("\n==========================================================\n");
fprintf("  ENGINE TRADE STUDY\n");
fprintf("  Question: which engine should the F-16A fly with?\n");
...
if ~isfile(physFile)
    error("F16AEngineTradeStudy:noPhysicalModel", ...
        "Missing %s. Run generate_f16a_physical first.", physFile);
end
if ~isfile(logiFile)
    error("F16AEngineTradeStudy:noLogicalModel", ...
        "Missing %s. Run generate_f16a_logical first.", logiFile);
end
if ~isfile(derFile)
    error("F16AEngineTradeStudy:noDecisionRequirements", ...
...
if ~isa(vc, "systemcomposer.arch.VariantComponent")
    error("F16AEngineTradeStudy:notAVariant", ...
...
for i = 1:n
    ...
    % silently left out of its own trade: add a fourth engine to the generator
    % and it is scored here, or this run stops and says which one is missing.
    applied = reshape(string(getStereotypes(c)), 1, []);
    if ~any(applied == profileName + "." + STEREOTYPE)
        error("F16AEngineTradeStudy:candidateNotStereotyped", ...
    ...
    fprintf("  found: %-26s kind=%-13s thrust=%7g lbf  mass=%8.2f lb  TRL=%d  (%s)\n", ...
        names(i), kinds(i), raw(i,1), raw(i,2), raw(i,3), provs(i));
end

% --- Refuse to trade on numbers that cannot be trusted ----------------
if n < 2
    error("F16AEngineTradeStudy:tooFewCandidates", ...
...
for i = 1:n
    if ~isfinite(raw(i,1)) || raw(i,1) <= 0
        error("F16AEngineTradeStudy:badThrust", ...
            "%s has Thrust_SL_lb = %g. It must be a positive number -- it is the " + ...
            "numerator of T/T_baseline, so an unset thrust scores as an engine that " + ...
            "produces none.", names(i), raw(i,1));
    end
    if ~isfinite(raw(i,2)) || raw(i,2) <= 0
        error("F16AEngineTradeStudy:badMass", ...
    ...
    if ~isfinite(raw(i,3)) || mod(raw(i,3),1) ~= 0 || raw(i,3) < 1 || raw(i,3) > 9
        error("F16AEngineTradeStudy:badTRL", ...
...
% --- The baseline the two ratio criteria divide by --------------------
isRef = provs == "Reference";
if sum(isRef) ~= 1
    error("F16AEngineTradeStudy:baselineNotUnique", ...
        "%d candidates are tagged DataProvenance = Reference; exactly one is required. " + ...
        "It is the baseline every ratio value function divides by, so with none there " + ...
        "is no scale and with two there is no answer to which one.", sum(isRef));
end
baseline = raw(isRef, :);
fprintf("  baseline    : %s -- the one candidate tagged DataProvenance = Reference\n", ...
    names(isRef));
fprintf("                thrust %g lbf, mass %.2f lb. It scores exactly 1.0 on both " + ...
    "ratio criteria, by construction.\n", baseline(1), baseline(2));
...
if abs(sum(w) - 1) > 1e-12
    error("F16AEngineTradeStudy:weightsDoNotSumToOne", ...
...
for k = 1:nCrit
    fprintf("  criterion : %-13s weight %.2f   v = %-14s (%s is better)\n", ...
...
if any(~isfinite(V(:)))
    error("F16AEngineTradeStudy:badValue", ...
...
% unbounded ratio criteria can reach here. Capping would throw away a genuine
% advantage and erroring would reject a legitimate candidate; the honest option
% left is to say that the declared weights have stopped describing influence.
for i = 1:n
    for k = 1:nCrit
        if V(i,k) > 1 + 1e-9
            warning("F16AEngineTradeStudy:valueAboveCeiling", ...
...
if abs(sorted(1) - sorted(2)) <= 1e-9
    error("F16AEngineTradeStudy:tie", ...
...
% Keyed on the winner's KIND, never its name: two engines both realize
% SingleEngine, so the candidate name is not a valid key into the L option set.
...
if isempty(req)
    error("F16AEngineTradeStudy:missingRequirement", ...
...
fprintf("  Saved: %s.slx, %s.slx, f16a_logical_derived.slreqx and the L link set.\n", ...
    physName, logiName);
fprintf("  Nothing was deleted. The rejected engines stay in the model, each carrying\n");
...
hdr = pad("Candidate", 26) + " " + pad("Kind", 14);
for k = 1:numel(crit)
    hdr = hdr + " " + pad(crit(k).Name, 13, "left");
end
for k = 1:numel(crit)
    hdr = hdr + " " + pad("v(" + crit(k).Name + ")", 16, "left");
end
...
for i = 1:numel(names)
    j = order(i);
    line = pad(names(j), 26) + " " + pad(kinds(j), 14);
    for k = 1:numel(crit)
        line = line + " " + pad(sprintf("%g", raw(j,k)), 13, "left");
    end
    for k = 1:numel(crit)
        line = line + " " + pad(sprintf("%.5f", V(j,k)), 16, "left");
    end
...
%TERMS "Thrust_SL_lb (-0.10387), Mass_lb (+0.09132), TRL (+0.08750)".
...
if ~isa(lvc, "systemcomposer.arch.VariantComponent")
    error("F16AEngineTradeStudy:logicalRoleNotVariant", ...
...
if ~any(names == kindName)
    error("F16AEngineTradeStudy:noSuchKind", ...
