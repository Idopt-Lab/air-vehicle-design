function ranked = F16AAirframeTradeStudy()
%F16AAIRFRAMETRADESTUDY Choose the F-16A airframe and write the choice into the model.
%   RANKED = F16AAIRFRAMETRADESTUDY() scores every choice of the Airframe variant
%   on aerodynamic merit, mass and maturity, picks the best, and records that
%   decision in the Physical model, the Logical model and REQ_F16A_L03.
%
...
%   F16AEngineTradeStudy and F16AFlightControlsTradeStudy; F16APhysicalTradeStudy
...
% file is mechanics.
% ---------------------------------------------------------------------
VARIANTPATH = "F16A_Physical/Aircraft/Airframe";   % the P variant holding the candidates
LOGICALROLE = "Airframe";                          % the L variant holding the kinds
DECISIONREQ = "REQ_F16A_L03";                      % where the decision is recorded
STEREOTYPE  = "AirframeCandidate";                 % the properties this trade reads
...
% AeroBenefit carries half the score, which is why STEP 2 boxes it at BOTH ends:
% 65 typed for 6.5 is finite, so no isfinite check would catch it (D-033).
crit = struct( ...
    "Name",   {"AeroBenefit", "Mass_lb",       "TRL"}, ...
    "Weight", {0.50,          0.25,            0.25}, ...
    "Value",  {@(x,b) x/10,   @(x,b) b/x,      @(x,b) (x-1)/8}, ...
    "Rule",   {"B/10",        "M_baseline/M",  "(TRL-1)/8"}, ...
    "Better", {"better aero", "less mass",     "more mature"});
nCrit = numel(crit);
...
fprintf("\n==========================================================\n");
fprintf("  AIRFRAME TRADE STUDY\n");
fprintf("  Question: what shape of airframe should the F-16A have?\n");
...
if ~isfile(physFile)
    error("F16AAirframeTradeStudy:noPhysicalModel", ...
        "Missing %s. Run generate_f16a_physical first.", physFile);
end
if ~isfile(logiFile)
    error("F16AAirframeTradeStudy:noLogicalModel", ...
        "Missing %s. Run generate_f16a_logical first.", logiFile);
end
if ~isfile(derFile)
    error("F16AAirframeTradeStudy:noDecisionRequirements", ...
...
if ~isa(vc, "systemcomposer.arch.VariantComponent")
    error("F16AAirframeTradeStudy:notAVariant", ...
...
for i = 1:n
    ...
    % silently left out of its own trade: add a third airframe to the generator
    ...
    if ~any(applied == profileName + "." + STEREOTYPE)
        error("F16AAirframeTradeStudy:candidateNotStereotyped", ...
    ...
    fprintf("  found: %-24s kind=%-22s aero=%4.1f/10  mass=%8.2f lb  TRL=%d  (%s)\n", ...
        names(i), kinds(i), raw(i,1), raw(i,2), raw(i,3), provs(i));
end

% --- Refuse to trade on numbers that cannot be trusted ----------------
if n < 2
    error("F16AAirframeTradeStudy:tooFewCandidates", ...
...
for i = 1:n
    if ~isfinite(raw(i,1)) || raw(i,1) < 1 || raw(i,1) > 10
        error("F16AAirframeTradeStudy:badAeroBenefit", ...
            "%s has AeroBenefit = %g. It must be on the declared 1..10 scale; the " + ...
            "default 0 means 'not set'. It is bounded at BOTH ends because v = B/10 " + ...
            "carries half the score and 95 typed for 9.5 is finite (D-033).", ...
            names(i), raw(i,1));
    end
    if ~isfinite(raw(i,2)) || raw(i,2) <= 0
        error("F16AAirframeTradeStudy:badMass", ...
    ...
    if ~isfinite(raw(i,3)) || mod(raw(i,3),1) ~= 0 || raw(i,3) < 1 || raw(i,3) > 9
        error("F16AAirframeTradeStudy:badTRL", ...
...
if sum(isRef) ~= 1
    error("F16AAirframeTradeStudy:baselineNotUnique", ...
...
if abs(sum(w) - 1) > 1e-12
    error("F16AAirframeTradeStudy:weightsDoNotSumToOne", ...
...
for k = 1:nCrit
    fprintf("  criterion : %-12s weight %.2f   v = %-14s (%s is better)\n", ...
...
if any(~isfinite(V(:)))
    error("F16AAirframeTradeStudy:badValue", ...
...
for i = 1:n
    for k = 1:nCrit
        if V(i,k) > 1 + 1e-9
            warning("F16AAirframeTradeStudy:valueAboveCeiling", ...
...
if abs(sorted(1) - sorted(2)) <= 1e-9
    error("F16AAirframeTradeStudy:tie", ...
...
if isempty(req)
    error("F16AAirframeTradeStudy:missingRequirement", ...
...
fprintf("  Saved: %s.slx, %s.slx, f16a_logical_derived.slreqx and the L link set.\n", ...
    physName, logiName);
fprintf("  Nothing was deleted. The rejected airframe stays in the model carrying a\n");
fprintf("  rationale saying what it lost on and by how much (D-002).\n\n");
...
hdr = pad("Candidate", 24) + " " + pad("Kind", 22);
for k = 1:numel(crit)
    hdr = hdr + " " + pad(crit(k).Name, 12, "left");
end
for k = 1:numel(crit)
    hdr = hdr + " " + pad("v(" + crit(k).Name + ")", 15, "left");
end
...
for i = 1:numel(names)
    j = order(i);
    line = pad(names(j), 24) + " " + pad(kinds(j), 22);
    for k = 1:numel(crit)
        line = line + " " + pad(sprintf("%g", raw(j,k)), 12, "left");
    end
    for k = 1:numel(crit)
        line = line + " " + pad(sprintf("%.5f", V(j,k)), 15, "left");
    end
...
%TERMS "AeroBenefit (+0.15000), Mass_lb (+0.01976), TRL (-0.03125)".
...
if ~isa(lvc, "systemcomposer.arch.VariantComponent")
    error("F16AAirframeTradeStudy:logicalRoleNotVariant", ...
...
if ~any(names == kindName)
    error("F16AAirframeTradeStudy:noSuchKind", ...
