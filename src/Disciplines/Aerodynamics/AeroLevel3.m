classdef AeroLevel3
     ...
     methods (Static)
          ...
          function CL_minD = compute_CL_minD(CL_alpha, alpha_L0_deg)
               ...
               CL_minD = CL_alpha*(-1*alpha_L0_rad/2);
          end

          % Get CL_alpha for wing + body
          function output = compute_CL_alpha_wb(CL_alpha_HT, CL_alpha_strakes, delta_epsilon_delta_alpha, S_HT, S_ref)
               output = CL_alpha_strakes + CL_alpha_HT*(1-delta_epsilon_delta_alpha)*(S_HT/S_ref);
          end

          % Get CL_alpha, accounting for strakes
          function output = compute_CL_alpha_strakes(CL_alpha_w, S_ref, S_strakes)
          ...
          function CL_alpha = get_CL_alpha_Raymer(statevector, S_exposed, S_ref, Lambda_max_t, Lambda_LE_deg, AR, fuselage_width, b)
               M = statevector(1);
          ...
          %    K1.subsonic
          %    K1.supersonic
          function K1 = compute_K1(e_osw, AR, M, Lambda_LE_degrees)
          ...
          % Compute K2
          function K2 = compute_K2(K1, CL_minD)
               K2.subsonic = -2 * K1.subsonic * CL_minD; % Brandt, cell G17
               K2.supersonic = 0;
          end
     ...
     % methods (Access = private)
          ...
          function output = get_V_and_mu(M, h_ft)
               ...
               mu = AeroLevel3.compute_dynamicviscosity(T);   % dynamic viscosity
               output = [V, mu, rho];
          end

          % Compute dynamic viscosity (mu) (should probably be in utilities...)
          function mu = compute_dynamicviscosity(T)
