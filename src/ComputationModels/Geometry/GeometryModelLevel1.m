classdef (Abstract) GeometryModelLevel1 < handle
     ...
     methods (Abstract)
          ...
          [c_HT, c_VT] = est_tail_propers(geometry_obj, aircraft_type, tail_type)
          S_ref_mainwing = get_wing_area(geometry_obj, W_TO, WS_desired) % Figure out
          % the logistics of this. Update from "Size aircraft" or from
          % here?
          [HT, VT] = tab_tail_AR_lambda(geometry_obj, aircraft_type, tail_type)
     end
end