...
- Constructors that load a JSON input file: give them a sensible default path (the checked-in per-fidelity JSON) with an optional override argument, so existing no-arg call sites (`F16GeomL1()`) in tests keep working.
