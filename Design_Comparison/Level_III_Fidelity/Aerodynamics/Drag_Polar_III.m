function [DragResults] = Drag_Polar_III(Designgeo_wings, Designgeo_fuselage, Designgeo_propulsion, Weight_Results)
...
FF_1 = @(x_c, t_c, M, Lambda_m) (1 + 0.6/(x_c)*(t_c) + 100*(t_c)^4)*(1.34*M^(0.18) * cos(Lambda_m)^0.28); % Raymer, eq 12.30, 6th edition

% Components: Fuselage, smooth canopy
FF_2 = @(l, d, A_max) (0.9 + 5/(f(l,d,A_max)^(1.5)) + f(l,d,A_max)/400); % Raymer, eq 12.31, 6th edition

% Components: Nacelle and smooth external store:
FF_3 = @(l, d, A_max) (1 + (0.35/f(l,d,A_max))); % Raymer, eq 12.32, 6th edition
...
k = 2.08*10^(-5); % smooth paint assumed.
...
Dq_upsweep = @(u,A_max) (3.83*u^(2.5)*A_max);

% "Base" areas drag area
Dq_base_sub = @(M, A_base) ((0.139 + 0.419*(M - 0.161)^2)*A_base);
Dq_base_sup = @(M, A_base) ((0.064 + 0.042*(M - 3.84)^2)*A_base);


% Windmilling engine drags
Dq_windmillingjet = @(A_engine_front_face) (0.3*A_engine_front_face);


% CALCULATING DRAG AREAS
Dq_upsweep_value = Dq_upsweep(0, pi*(D_fus/2)^2);
Dq_windmillingjet_value = Dq_windmillingjet(pi*((34.8/12)/2)^2);
...
% Compute WAVE DRAG
Lambda_le_deg = 40;
Dq_searshaack = @(A_max, l) (9*pi/2 * (A_max/l)^2);
Dq_wave = @(E_WD, M, Lambda_LE_deg, A_max, l) (E_WD*(1-0.386*(M-1.2)^(0.57)*(1 - (pi*Lambda_le_deg^0.77)/100))*(Dq_searshaack(A_max, l)));
...
disp("CD0 sub: " + CD0_sub)
disp("CD0 sup: " + CD0_supersonic)
disp("M_DD: " + M_DD)
...
e_straight = @(AR) (1.78 * ( 1 - 0.045*AR^(0.68)) - 0.64); % For straight wings (sweep < 30 deg)
e_swept = @(AR) (4.61*(1-0.045*AR^(0.68))*cos(Lambda_le_deg*pi/180)^(0.15) - 3.1); % For swept-wing (sweep > 30 deg)
...
K_supersonic = ((AR*M^2 - 1) * cos(Lambda_le_deg*pi/180))/(4*AR*sqrt(M^2 - 1) - 2);
