...
function [Weight_Results] = weight_est_III(missiondata, Constraints, Designgeo_wings, Designgeo_fuselage, Designgeo_propulsion, min_TW, optimal_WS, DesignTable_weight)
...
%% --------------------------------------------------
% Run analysis
[W_TO, beta, results_table, total_fuel_used, fuel_fraction, empty_weight_fraction, empty_weight, W_Takeoff, W_Climb, W_Cruise, W_Dash, W_Combat, W_Cruise2, W_Loiter, W_Landing, f1, f2, f3, f4, f5, f6, f7, f8, S_ref, S_wet, T0, S_HT, S_VT] = missionAnalysis(mission, min_TW, optimal_WS, Designgeo_wings, Designgeo_fuselage, DesignTable_weight);
...
% Function: Main Mission Analysis
     function [W_TO, beta, results_table, total_fuel_used, fuel_fraction, empty_weight_fraction, empty_weight, W_Takeoff, W_Climb, W_Cruise, W_Dash, W_Combat, W_Cruise2, W_Loiter, W_Landing, f1, f2, f3, f4, f5, f6, f7, f8, S_ref, S_wet, T0, S_HT, S_VT] = missionAnalysis(mission, min_TW, optimal_WS, Designgeo_wings, Designgeo_fuselage, DesignTable_weight)
          ...
          BPR = Designgeo_propulsion.BypassRatio("Bypass Ratio");
          ...
          for iteration = 1:max_iteration
               ...
               % Estimate engine characteristics
               [enginestats] = propulsion_est_III(T0, mission.Dash.Dash("Mach number"), BPR);
               ...
               [W_Cruise, f5]  = segment_cruise(W_Climb, W_S, enginestats.SFC_cruise, mission.Cruise.Cruise("Range (ft)"), mission.Cruise.Cruise("Mach number"), mission.Cruise.Cruise("a (ft/s)"), mission.Cruise.Cruise("q (lbf/ft^2)"), mission.Cruise.Cruise("CD0"), mission.Cruise.Cruise("e"), AR, W_TO, S_ref);
               [W_Dash, f6]    = segment_dash(W_Cruise, W_S, W_TO, mission.Dash.Dash("q (lbf/ft^2)"), mission.Dash.Dash("CD0"), mission.Dash.Dash("e"), AR, enginestats.SFC_maxT, mission.Dash.Dash("Range (ft)"), mission.Dash.Dash("Mach number") * mission.Dash.Dash("a (ft/s)"));
               [W_Combat, f7]  = segment_combat(W_Dash, mission.Combat.Combat("Time (min)"), enginestats.SFC_maxT, mission.Combat.Combat("Payload, drop (lbf)"), mission.Combat.Combat("CD0"), mission.Combat.Combat("e"), AR, W_TO, W_S, mission.Combat.Combat("q (lbf/ft^2)"));
               [W_Cruise2, f8] = segment_cruise(W_Combat, W_S, enginestats.SFC_cruise, mission.Cruise_1.Cruise_1("Range (ft)"), mission.Cruise_1.Cruise_1("Mach number"), mission.Cruise_1.Cruise_1("a (ft/s)"), mission.Cruise_1.Cruise_1("q (lbf/ft^2)"), mission.Cruise_1.Cruise_1("CD0"), mission.Cruise_1.Cruise_1("e"), AR, W_TO, S_ref);
               [W_Loiter, f9]  = segment_loiter(W_TO, W_Cruise2, W_S, mission.Loiter.Loiter("q (lbf/ft^2)"), mission.Loiter.Loiter("CD0"), mission.Loiter.Loiter("e"), AR, mission.Loiter.Loiter("Time (min)"), mission.Loiter.Loiter("TSFC"));
               ...
               W_engine_installed = 1.3*Engine_Sizing(T0); % Installed engine weight (lbf)
               [empty_weight] = Compute_OEW_III(W_TO, S_ref, S_HT, S_VT, S_wet, T0);
...
Weight_Results.S_VT = S_VT;
Weight_Results.S_HT = S_HT;
